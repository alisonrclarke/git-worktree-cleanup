...
def _run_git_cmd(path, cmd, *args):
    result = subprocess.run(
        ["git", cmd] + list(args), check=True, cwd=path, capture_output=True
    )
...
def _iterate_options(
    worktree_dir: str,
    subdir: str,
    subdir_path: str,
    subdir_status: SubdirStatus,
    subdir_branch: str = None,
):
    next_step = None
...
def _is_branch_merged(
    repo: Repo, default_branch: str, worktree_dir: str, branch: str, branch_commit: str
) -> bool:
    ...
    if default_branch in containing_branches:
        ...
        merge_base = _run_git_cmd(
            worktree_dir, "merge-base", default_branch, branch
        ).rstrip()
        rev_parse_hash = _run_git_cmd(
            worktree_dir, "rev-parse", f"{branch}^{{tree}}"
        ).rstrip()
        commit_tree_hash = _run_git_cmd(
            worktree_dir, "commit-tree", rev_parse_hash, "-p", merge_base, "-m", "_"
        ).rstrip()
        cherry_output = _run_git_cmd(
            worktree_dir, "cherry", default_branch, commit_tree_hash
        )

        if cherry_output.startswith("-"):
            print(
                f"{branch} at commit {branch_commit} has been squashed-and-rebased to {default_branch}"
            )
            merged = True
        else:
            merged = False
...
def cleanup():
    ...
    if len(sys.argv) == 1:
        ...
        worktree_dir = prompt(
            "Select a directory that uses git-worktree: ",
            default="~/",
    ...
    remote_branches = sorted(
        [b.name.removeprefix("origin/") for b in repo.remote().refs]
    )
    default_branches = [b for b in remote_branches if default_branch_pattern.match(b)]

    default_branch = choice(
        message="Select default remote branch: ",
        default="develop",
        options=[(b, b) for b in default_branches],
    )
    ...
    for subdir in subdirs:
        ...
        try:
            ...
            _iterate_options(
                worktree_dir, subdir, subdir_path, SubdirStatus.NOT_A_WORKTREE
            )
            continue
        ...
        if merged:
            ...
            if modified_files or untracked_files:
                ...
                print(
                    f"There are {len(modified_files)} changed files and  {len(untracked_files)} changes in {subdir}"
                )
            ...
            print(
                f"{subdir_branch} at commit {worktree_repo.commit()} has not been merged to {default_branch}"
            )
    ...
    for branch_name, head in local_branches.items():
        merged = _is_branch_merged(
            repo, default_branch, worktree_dir, branch_name, head.commit.hexsha
        )
